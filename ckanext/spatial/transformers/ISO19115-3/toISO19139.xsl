<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="/">
    ...
    <xsl:for-each select="/*">
      ...
      <xsl:element name="{concat($nameSpacePrefix,':',local-name(.))}">
        ...
        <xsl:choose>
          <xsl:when test="mdb:dateInfo[cit:CI_Date/cit:dateType/cit:CI_DateTypeCode/@codeListValue='revision' or cit:CI_Date/cit:dateType/cit:CI_DateTypeCode='revision']">
            <xsl:apply-templates select="mdb:dateInfo[cit:CI_Date/cit:dateType/cit:CI_DateTypeCode/@codeListValue='revision' or cit:CI_Date/cit:dateType/cit:CI_DateTypeCode='revision']"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:apply-templates select="mdb:dateInfo[cit:CI_Date/cit:dateType/cit:CI_DateTypeCode/@codeListValue='creation' or cit:CI_Date/cit:dateType/cit:CI_DateTypeCode='creation']"/>
          </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:if test="name(preceding-sibling::node()[1]) != name()">
      <gmd:hierarchyLevel>
        <xsl:variable name="codeListValue">
          <xsl:choose>
             <xsl:when test="mdb:MD_MetadataScope/mdb:resourceScope/mcc:MD_ScopeCode/@codeListValue">
               <xsl:value-of select="mdb:MD_MetadataScope/mdb:resourceScope/mcc:MD_ScopeCode/@codeListValue"/>
             </xsl:when>
             <xsl:otherwise>
               <xsl:value-of select="mdb:MD_MetadataScope/mdb:resourceScope/mcc:MD_ScopeCode"/>
             </xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
        <xsl:element name="gmd:MD_ScopeCode">
          <xsl:attribute name="codeList" select="'http://standards.iso.org/ittf/PubliclyAvailableStandards/ISO_19139_Schemas/resources/codelist/ML_gmxCodelists.xml#MD_ScopeCode'"/>
  ...
  but implementations usually use date stamp as revision date. UPDATE: added choose to select create if revision not set to root-->
  <xsl:template match="mdb:dateInfo[cit:CI_Date/cit:dateType/cit:CI_DateTypeCode]" priority="5">
    <gmd:dateStamp>
  ...
  <xsl:template match="mri:resourceMaintenance" priority="5">
    <gmd:resourceMaintenance>
      <gmd:MD_MaintenanceInformation>
          <xsl:copy-of select="descendant::gmd:date/@gco2:nilReason"/>
            <xsl:call-template name="writeCodelistElement">
              <xsl:with-param name="elementName" select="'gmd:maintenanceAndUpdateFrequency'"/>
              <xsl:with-param name="codeListValue" select="mmi:MD_MaintenanceInformation/mmi:maintenanceAndUpdateFrequency/mmi:MD_MaintenanceFrequencyCode"/>
              <xsl:with-param name="codeListName" select="'gmd:MD_MaintenanceFrequencyCode'"/>
            </xsl:call-template>
      </gmd:MD_MaintenanceInformation>
  </gmd:resourceMaintenance>
  </xsl:template>
        ...
        <xsl:element name="{concat($nameSpacePrefix,':',local-name(.))}">
          ...
          <xsl:call-template name="writeCodelistElement">
            ...
            <xsl:with-param name="codeListValue">
              <xsl:choose>
                <xsl:when test="mri:status/mcc:MD_ProgressCode/@codeListValue">
                  <xsl:value-of select="mri:status/mcc:MD_ProgressCode/@codeListValue"/>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:value-of select="mri:status/mcc:MD_ProgressCode"/>
                </xsl:otherwise>
              </xsl:choose>
            </xsl:with-param>
